    ...
    function getMessageHandler(
        ...
        return {
            ...
            deletePullRequestFederatedCredentialRequest: (args) =>
                handleDeletePullRequestFederatedCredentialRequest(args.key, args.repositoryKey),
            deleteBranchFederatedCredentialRequest: (args) =>
                handleDeleteBranchFederatedCredentialRequest(args.key, args.repositoryKey, args.branch),
            ...
            deleteRepoSecretRequest: (args) => handleDeleteRepoSecretsRequest(args.key, args.secret),
            ...
            deleteAcrRoleAssignmentRequest: (args) =>
                handleDeleteAcrRoleAssignmentRequest(args.acrKey, args.servicePrincipalKey, args.roleDefinitionKey),
            deleteClusterRoleAssignmentRequest: (args) =>
                handleDeleteClusterRoleAssignmentRequest(
                    args.clusterKey,
                    args.servicePrincipalKey,
                    args.roleDefinitionKey,
                ),
            createEntraIdApplicationRequest: (args) => handleCreateEntraIdApplicationRequest(args.applicationName),
            deleteEntraIdApplicationRequest: (args) => handleDeleteEntraIdApplicationRequest(args),
        };
        ...
        async function handleDeletePullRequestFederatedCredentialRequest(
            key: EntraIdApplicationKey,
            repoKey: GitHubRepoKey,
        ) {
            await delay(2000);
            const appData = referenceData.applications.find((a) => a.objectId === key.objectId);
            const credIndex = appData?.pullRequestFederatedCredentials.findIndex(
                (c) => c.owner === repoKey.gitHubRepoOwner && c.name === repoKey.gitHubRepoName,
            );
            if (credIndex !== undefined && credIndex >= 0) {
                appData?.pullRequestFederatedCredentials.splice(credIndex, 1);
            }
        ...
        async function handleDeleteBranchFederatedCredentialRequest(
            key: EntraIdApplicationKey,
            repoKey: GitHubRepoKey,
            branch: string,
        ) {
            await delay(2000);
            const appData = referenceData.applications.find((a) => a.objectId === key.objectId);
            const credIndex = appData?.branchFederatedCredentials.findIndex(
                (c) => c.owner === repoKey.gitHubRepoOwner && c.name === repoKey.gitHubRepoName && c.branch === branch,
            );
            if (credIndex !== undefined && credIndex >= 0) {
                appData?.branchFederatedCredentials.splice(credIndex, 1);
            }
        ...
        async function handleDeleteRepoSecretsRequest(repoKey: GitHubRepoKey, secret: GitHubRepoSecret) {
            await delay(2000);
            const repoData = referenceData.gitHubRepos.find(
                (r) => r.owner === repoKey.gitHubRepoOwner && r.name === repoKey.gitHubRepoName,
            );
            ...
            repoData!.secrets = secretState;
            webview.postDeleteRepoSecretResponse({ key: repoKey, secret, secretState });
        }
        ...
        async function handleDeleteAcrRoleAssignmentRequest(
            acrKey: AcrKey,
            servicePrincipalKey: ServicePrincipalKey,
            roleDefinitionKey: AcrRoleDefinitionKey,
        ) {
            ...
            const roleIndex = roleDefinitions.findIndex(
                (r) => r.roleDefinitionName === roleDefinitionKey.roleDefinitionName,
            );
            if (roleIndex !== undefined && roleIndex >= 0) {
                roleDefinitions.splice(roleIndex, 1);
            }
        ...
        async function handleDeleteClusterRoleAssignmentRequest(
            clusterKey: ClusterKey,
            servicePrincipalKey: ServicePrincipalKey,
            roleDefinitionKey: ClusterRoleDefinitionKey,
        ) {
        ...
        async function handleDeleteEntraIdApplicationRequest(key: EntraIdApplicationKey) {
            await delay(2000);
            const appIndex = referenceData.applications.findIndex((a) => a.objectId === key.objectId);
            if (appIndex !== undefined && appIndex >= 0) {
                referenceData.applications.splice(appIndex, 1);
            }
