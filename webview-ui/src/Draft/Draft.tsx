...
export function Draft(initialState: InitialState) {
    ...
    const lazySubscriptionData = prepareSubscriptionData(
        state.referenceData,
        state.azureResources.selectedSubscription,
        updates,
    );
    const lazyRepositoryData = prepareRepositoryData(
        lazySubscriptionData,
        state.azureResources.repositoryDefinition,
        updates,
    );
    ...
    const selectedServiceState = state.selectedService
        ? state.services.find((s) => s.name === state.selectedService)
        : null;
        ...
        <>
            ...
            */}
            ...
            <div className={styles.inputContainer}>
                ...
                {state.services.length > 0 && (
                    <>
                        <label htmlFor="service-dropdown" className={styles.label}>
                            Service
                        </label>
...
): Lazy<Maybe<SubscriptionReferenceData>> {
    let returnValue: Lazy<Maybe<SubscriptionReferenceData>> = selectedSubscription
        ? newLoading()
        : newLoaded(nothing());

    if (!isLoaded(referenceData.subscriptions)) {
        loadSubscriptions(referenceData, updates);
        return returnValue;
    }

    if (selectedSubscription !== null) {
        const subscriptionData = tryGet(
            referenceData.subscriptions.value,
            (sub) => sub.subscription.id === selectedSubscription.id,
        );
...
function prepareRepositoryData(
    lazySubscriptionData: Lazy<Maybe<SubscriptionReferenceData>>,
    repositoryDefinition: SavedRepositoryDefinition | null,
    updates: EventHandlerFunc[],
): Lazy<Maybe<RepositoryReferenceData>> {
    let returnValue: Lazy<Maybe<RepositoryReferenceData>> = repositoryDefinition
        ? newLoading()
        : (newLoaded(nothing()) as Lazy<Maybe<RepositoryReferenceData>>);
    ...
    const resourceGroupData = tryGet(
        lazyResourceGroupsData.value,
    ...
    if (!isLoaded(lazyAcrsData)) {
        loadAcrs(resourceGroupData.value, updates);
        return returnValue;
    }
    ...
    if (!isLoaded(lazyRepositoriesData)) {
        loadRepositories(acrData.value, updates);
        return returnValue;
    }
    ...
    if (!isLoaded(lazyBuiltTags)) {
        loadBuiltTags(repositoryData.value, updates);
    }
...
function prepareClusterData(
    ...
    updates: EventHandlerFunc[],
): Lazy<Maybe<ClusterReferenceData>> {
    let returnValue: Lazy<Maybe<ClusterReferenceData>> = clusterDefinition
        ? newLoading()
        : (newLoaded(nothing()) as Lazy<Maybe<ClusterReferenceData>>);
    ...
    if (!isLoaded(lazyClustersData)) {
        loadClusters(resourceGroupData.value, updates);
        return returnValue;
    }
    ...
    const lazyConnectedAcrs = clusterData.value.connectedAcrs;
    if (!isLoaded(lazyConnectedAcrs)) {
        loadConnectedAcrs(clusterData.value, updates);
    }
